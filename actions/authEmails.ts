...
const recoveryEmailTemplate = (email: string, token: string) => {
  return `<div style="margin: 10px;">
  <h2>Recover Your Password</h2><p>Press the button below to proceed with recovering your password for <b>Mind Anchor</b>.</p> 
<a href="${process.env.NEXT_PUBLIC_API_BASE_URL}/auth/recover/new-password?token=${token}&email=${email}" 
style="padding: 10px; background-color: blue; color: white; text-decoration: none; border-radius: 5px;">Validate Email</a>
  </div>
`;
};

export const sendRecoveryEmail = async (token: string, email: string) => {
  const info = await transporter.sendMail({
    from: '"Daniil Rayu @ Mind Anchor" <dan.rayu@gmail.com>', // sender address
    to: email,
    subject: "Recover your password", // Subject line
    html: recoveryEmailTemplate(email, token), // html body
  });
};
